...
import { NORMAL_TEST_DURATION_MS, EXTREME_TEST_DURATION_MS, MAX_PARTICLES } from './constants';
...
const App: React.FC = () => {
  ...
  // selectedDuration is kept for potential UI indication, but core logic uses benchmarkRunningDurationRef
  const [selectedDuration, setSelectedDuration] = useState<number>(NORMAL_TEST_DURATION_MS); 
  ...
  // New ref to hold the *actual* duration for the currently running benchmark
  const benchmarkRunningDurationRef = useRef<number>(NORMAL_TEST_DURATION_MS);
  ...
  const updatePhysics = (dt: number) => {
      ...
      const stride = len > 8000 ? Math.ceil(len / 4000) : 1; // Keep to help UI responsiveness at high loads

      for (let i = 0; i < len; i++) {
          const bodyA = bodies[i];

          // Center Attraction (Keep the swarm in the middle of the screen)
          // Slightly reduce the pull to allow for a wider spread and more interactions
          const centerPull = 0.000008; 
  ...
  const startBenchmark = useCallback(() => {
    ...
    // Get the actual duration for this benchmark run from the ref
    const currentDuration = benchmarkRunningDurationRef.current;
    ...
    const addParticles = (count: number) => {
        for(let i=0; i<count; i++) {
            if (bodiesRef.current.length >= MAX_PARTICLES) return; // Prevent exceeding cap
    ...
    const endTime = startTime + currentDuration; // Use currentDuration from ref
    ...
    const loop = () => {
        ...
        if (now > lastMetricTime + 500) {
            ...
            setProgress((elapsed / currentDuration) * 100); // Use currentDuration for progress
            ...
            // Make ramping more aggressive for difficulty
            const rampAmount = currentFps > 45 ? 2000 : (currentFps < 15 ? 200 : 800);
        ...
        const stressLevel = Math.min(1, len / (MAX_PARTICLES * 0.5)); // Scale color shift to new MAX_PARTICLES
    ...
    animationFrameIdRef.current = requestAnimationFrame(loop);
  }, [isRunning]); // selectedDuration removed from dependencies
  ...
  const finish = useCallback(() => {
     ...
     // Use the duration from the ref for score calculation
     const actualBenchmarkDuration = benchmarkRunningDurationRef.current;
     const score = Math.floor((totalCalcsRef.current / (actualBenchmarkDuration === EXTREME_TEST_DURATION_MS ? 500000 : 100000)) * (avg / 60));

     setHistory(data);
     setAvgFps(Math.round(avg));
     setPeakParticles(peak);
     setFinalScore(score);
  }, []); // selectedDuration removed from dependencies
  ...
  const goToStartScreen = useCallback(() => {
    ...
    // Clear the canvas when returning to the start screen
    const ctx = canvasRefs.current?.ctx;
    if (ctx) {
  ...
  const Graph = ({ data }: { data: DataPoint[] }) => {
      ...
      // The graph's end time is derived directly from the collected data,
      // ensuring it correlates with the actual selected benchmark duration (e.g., 60s for normal, 1200s for extreme).
      ...
      <div className="absolute inset-0 pointer-events-none z-20 flex flex-col p-6 font-sans select-none">
         ...
         <div className="flex justify-between items-center mb-8">
            <button 
                ...
                className="flex items-center gap-3 pointer-events-auto cursor-pointer focus:outline-none focus:ring-2 focus:ring-lime-500 focus:ring-offset-2 focus:ring-offset-black rounded-lg p-2 -ml-2 transition-all hover:scale-[1.02]"
             ...
             <div className="flex-1 flex items-center justify-center pointer-events-auto">
                 <div className="text-center max-w-lg">
                     ...
                     <div className="flex flex-col sm:flex-row gap-4 justify-center">
                         <button 
                            onClick={() => { 
                                setSelectedDuration(NORMAL_TEST_DURATION_MS); 
                                benchmarkRunningDurationRef.current = NORMAL_TEST_DURATION_MS;
                                startBenchmark(); 
                            }}
                            className="group relative bg-lime-600 hover:bg-lime-500 text-white font-semibold text-lg py-5 px-8 rounded-full shadow-[0_0_50px_rgba(101,163,13,0.4)] transition-all hover:scale-105 active:scale-95 overflow-hidden"
                         >
                            ...
                            <span>Run Normal (1 Min)</span>
                         </button>
                         <button 
                            onClick={() => { 
                                setSelectedDuration(EXTREME_TEST_DURATION_MS); 
                                benchmarkRunningDurationRef.current = EXTREME_TEST_DURATION_MS;
                                startBenchmark(); 
                            }}
                            className="group relative bg-red-600 hover:bg-red-500 text-white font-semibold text-lg py-5 px-8 rounded-full shadow-[0_0_50px_rgba(220,38,38,0.4)] transition-all hover:scale-105 active:scale-95 overflow-hidden"
                         >
                            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent skew-x-[-20deg] translate-x-[-150%] group-hover:translate-x-[150%] transition-transform duration-700 ease-in-out"></div>
                            <span>Run EXTREME (20 Mins)</span>
                         </button>
                     </div>
                 </div>
             </div>
